...
const char placeholder1[] = "Public Key";  // Add another persons public key.  
const char placeholder2[] = "Send";
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (msg) {
        ...
        case WM_CREATE:
            // Make it a text input field
            hEdit2 = CreateWindowEx(WS_EX_CLIENTEDGE, "EDIT", "", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL,
        ...
        case WM_COMMAND:
            if (LOWORD(wParam) == ID_EDIT2) {
                char keyBuffer[256];
                GetWindowText(hEdit2, keyBuffer, sizeof(keyBuffer));
                std::string publicKey = keyBuffer;

            if (publicKey.empty()) {
                MessageBox(hwnd, "Please enter a public key.", "Warning", MB_OK | MB_ICONWARNING);
                return 0;
            }

            // Simulate sending a request (create a request file with their public key)
            std::string filePath = "requests\\" + publicKey + ".req";
            std::ofstream requestFile(filePath);

            if (requestFile.is_open()) {
                requestFile << "From: " << username << "\n";
                requestFile << "To: " << publicKey << "\n";
                requestFile << "Message: Request to connect.\n";
                requestFile.close();

                MessageBox(hwnd, "Request sent!", "Success", MB_OK | MB_ICONINFORMATION);
            } else {
                MessageBox(hwnd, "Failed to write request file.", "Error", MB_OK | MB_ICONERROR);
            }
